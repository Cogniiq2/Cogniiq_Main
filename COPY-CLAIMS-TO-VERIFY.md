...
| Z12 | **Blog-Orientierungspreise** — der einzige Betrag, der den bestätigten `TARIFE` widersprach (200–450 € für den Telefonassistenten), ist am 18.08.2026 entfernt. Offen bleiben die Webdesign- und Automatisierungsbeträge (150–500 €, 2.500–8.000 € u. a.) | `blog-data.ts` | Bestätigen oder entfernen. **Abgrenzung 12.09.2026:** Diese Zeile betrifft ab jetzt nur noch `blog-data.ts`. Die Automatisierungsbeträge auf den **Verkaufsflächen** sind unter Z26 abgeschlossen, die Webdesign-Beträge auf den Verkaufsflächen unter **Z28** |
...
| Z27 | **Häufigkeitsschwelle „zwölfmal im Jahr"** — **erledigt 13.09.2026**, aufgefallen im Inhaber-Review. Beim Entfernen der unbelegten Automatisierungsbeträge (Z26) war eine neue unbelegte Zahl entstanden: eine Frequenzgrenze, ab der sich Automatisierung angeblich nicht mehr trägt. Häufigkeit allein entscheidet das nicht — Aufwand, Wert und Risiko je Durchlauf gegen Investition und laufende Kosten entscheiden es. Ersatzlos ersetzt durch die korrekte Logik; zusätzlich „in zehn Sätzen" → „in wenigen Sätzen" | `pages/ProzessautomatisierungHub.tsx`, `pages/costs/KostenAutomatisierung.tsx` | **Merksatz für künftige Durchgänge:** Eine Zahl, die eine Leistung *begrenzt*, braucht denselben Beleg wie eine, die sie *verspricht* |
| Z28 | **Webdesign-Preise, -Dauern und -Ergebniszusagen auf den Verkaufsflächen** — **erledigt 13.09.2026**: auf `/kosten-webdesign` alle achtzehn unbelegten Beträge/Zusagen (Staffeln ab 1.500 / 2.500–5.000 / ab 5.000 €, vier Beispielprojektpreise, Wartung 50–150 €/Monat, Local SEO 500–2.000 €, Terminbuchung 800–2.500 €, Texte 80–150 €/Seite, „30 Tage gültig", Ratenmodell, `Offer.price`) entfernt und **nicht ersetzt**; auf `/webdesign` „hochkonvertierend", „ranken", „7–14 Tage", „unter 2 Sek.", „Festpreise", „ohne Rankingverlust", „keine Templates" entfernt; `/webdesign-agentur-deutschland` trägt „ab 1.500 €", „4–14 Wochen", „A/B-getestet", „nachweislich besser ranken" weiter, bis F9 sie per 301 ablöst (Anker in eingefrorene Experimente); auf `/regensburg/webdesign`, `/muenchen/webdesign`, `/bayreuth/lokales-seo` FAQ-Preise und -Dauern ersetzt. Herkunftsprüfung: `docs/seo/preisaudit-webdesign.md` | `pages/WebdesignHub.tsx`, `pages/costs/KostenWebdesign.tsx`, `standorte-service-configs.ts`, `cluster/bayreuth/LokalesSEOBayreuth.tsx` | **Offen bleibt:** `/bayreuth/webdesign` und `/muenchen/webdesign-kosten` tragen dieselben Aussagen eingefroren weiter (post-experiment-opportunities.md); `/webdesign-hotel` „amortisiert sich in 6–12 Monaten" ist Messpunkt M1 — Inhaber-Entscheidung. Sobald der Inhaber Webdesign-Preise bestätigt: kanonische Quelle nach Muster `TARIFE` |
