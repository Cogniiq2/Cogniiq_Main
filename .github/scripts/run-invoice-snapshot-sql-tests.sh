...
# the new invoice-immutable-snapshot migration (Phase 1A), then EXECUTE every issuance/deletion/
# cancellation RPC it touches. Tears the cluster down afterwards. Never touches Supabase, never
# sends mail, never opens a network listener.
#
...
# Same chain as run-finance-multipay-sql-tests.sh, plus the new Phase 1A migration at the end.
MIGRATIONS=(
...
PSQL -c "create database invsnap;" >/dev/null
PSQL -d invsnap -q -f "$ROOT/supabase/tests/lib_bootstrap.sql" >/dev/null

# lib_bootstrap.sql installs pgcrypto with no explicit schema (lands in `public` on a fresh
# cluster). The functions touched by this migration call `extensions.digest(...)` explicitly
# (the same Supabase-compatible pattern 20260723124000_owner_premium_offer_runtime_hotfix.sql
# already established for finalize_owner_offer) — on real Supabase pgcrypto already lives in
# `extensions`, so this relocation only matters for this local throwaway cluster.
PSQL -d invsnap -q -c "create schema if not exists extensions; alter extension pgcrypto set schema extensions;" >/dev/null

# Minimal Storage surface, mirroring run-finance-multipay-sql-tests.sh, so the customer-documents
# migration can register its bucket exactly as it does in production.
PSQL -d invsnap -q <<'SQL' >/dev/null
...
for f in "${MIGRATIONS[@]}"; do
  PSQL -d invsnap -q -f "$MIG/$f.sql" >/dev/null
done

PSQL -d invsnap -f "$SQLDIR/finance-invoice-snapshot-tests.sql"

# Convergence: the new migration must be safely re-appliable and the tests must still pass.
PSQL -d invsnap -q -f "$MIG/20260830123000_owner_invoice_immutable_snapshot.sql" >/dev/null
PSQL -d invsnap -f "$SQLDIR/finance-invoice-snapshot-tests.sql" >/dev/null
echo "migration convergence: 20260830123000 re-applied cleanly and invoice snapshot tests still pass"
