...
# the invoice-integrity-guard migration (PR-0A), then EXECUTE the guard, the grants and both
# offer->invoice conversion paths against it. Tears the cluster down afterwards. Never touches
# Supabase, never sends mail, never opens a network listener.
#
# Executable rather than source-parsing on purpose: a text-level test cannot tell a REVOKE that
# was written from one that took effect, nor a guard that COMPILES from one that refuses.
set -euo pipefail
...
# Same chain as run-invoice-snapshot-sql-tests.sh, plus the PR-0A guard migration at the end.
MIGRATIONS=(
...
PSQL -d invintegrity -f "$SQLDIR/invoice-integrity-tests.sql" >/dev/null
echo "migration convergence: 20260831120000 re-applied cleanly and invoice integrity tests still pass"
