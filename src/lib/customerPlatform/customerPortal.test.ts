...
describe('buildRailProjectChildren', () => {
  it('links each project to its real detail route', () => {
    const children = buildRailProjectChildren([{ id: 'p1', title: 'Rollout', status: 'on_track' }]);
    expect(children).toEqual([{ label: 'Rollout', href: '/app/projects/p1', tone: undefined }]);
  });

  it('never invents a /app/projects index', () => {
    const children = buildRailProjectChildren([{ id: 'p1', title: 'X', status: 'on_track' }]);
    expect(children.every((c) => c.href !== '/app/projects')).toBe(true);
  });

  it('leaves completed projects out of the rail', () => {
    expect(buildRailProjectChildren([{ id: 'p1', title: 'X', status: 'completed' }])).toHaveLength(0);
  });

  it('tones a blocked and an attention-needing project differently', () => {
    const [blocked, attention] = buildRailProjectChildren([
      { id: 'a', title: 'A', status: 'blocked' },
      { id: 'b', title: 'B', status: 'attention_needed' },
    ]);
    expect(blocked.tone).toBe('danger');
    expect(attention.tone).toBe('attention');
  });

  it('caps the rail so a customer with many projects does not get an endless list', () => {
    const many = Array.from({ length: 12 }, (_, i) => ({ id: `p${i}`, title: `P${i}`, status: 'on_track' }));
    expect(buildRailProjectChildren(many)).toHaveLength(5);
  });

  it('returns nothing rather than a placeholder when there is no project', () => {
    expect(buildRailProjectChildren([])).toEqual([]);
  });
});
