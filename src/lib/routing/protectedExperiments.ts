...
//     check binds the two: scripts/prerender.mjs asserts, for
...
export function countPathOccurrences(
  ...
  for (const [file, content] of sources) {
    // The scheme+host of an absolute URL is stripped first, so
    // `https://cogniiq.de/kosten-ki-telefonassistent` is counted like the
    // relative link it is equivalent to. Without this the left-hand lookbehind
    // sees the `e` of the host and refuses the match — which meant every
    // absolute link to a frozen route was invisible to this guard, including
    // the six <loc> entries in public/sitemap.xml that the file list below
    // deliberately scans. Over-counting an unrelated host that happens to use
    // the same path is the safe direction for a freeze: it raises a question
    // instead of silently missing a new inbound link.
    const normalised = content.replace(/\bhttps?:\/\/[a-z0-9.-]+/gi, '');
    // Compiled once above; lastIndex is reset per file because /g is stateful.
    pattern.lastIndex = 0;
    const hits = normalised.match(pattern)?.length ?? 0;
