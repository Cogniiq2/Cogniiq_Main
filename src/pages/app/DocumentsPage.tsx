            ...
            {showFilters ? (
              <div className="rounded-card border border-hairline bg-white p-4 shadow-card">
                <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_190px_190px]">
                  ...
                  {/* Options carry the German category LABEL, never the stored token. An
                      unmapped category falls back to an em dash rather than printing the enum. */}
                  <AppSelect
                    id="document-category-filter"
                    label="Nach Kategorie filtern"
                    labelHidden
                    value={category}
                    onChange={(value) => setCategory(value as CustomerDocumentCategory | typeof ALL)}
                    options={[
