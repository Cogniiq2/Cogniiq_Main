...
import { Home, Briefcase, Users, HelpCircle, Mail } from 'lucide-react';
...
const InteractiveMenu: React.FC<InteractiveMenuProps> = ({ items, accentColor, onNavigate }) => {
  ...
  const [activeIndex, setActiveIndex] = useState(() => {
    const currentIndex = finalItems.findIndex(item => item.path === location.pathname);
  ...
  const handleItemClick = (index: number) => {
    ...
    navigate(finalItems[index].path);
    if (onNavigate) {
      onNavigate();
    }
  };
