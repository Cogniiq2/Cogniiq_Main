...
export function Modal({ open, onClose, title, description, children, footer, size = 'md' }: ModalProps) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[90] flex items-end justify-center sm:items-center" role="presentation">
      <div className="absolute inset-0 bg-gray-950/35 backdrop-blur-[2px] animate-in fade-in duration-150 motion-reduce:animate-none" onClick={onClose} aria-hidden="true" />
      <div
        ref={trapRef}
        role="dialog"
        aria-modal="true"
        aria-label={title}
        className={cn(
          'relative w-full rounded-t-panel bg-white shadow-overlay sm:rounded-panel',
          'animate-in fade-in slide-in-from-bottom-4 duration-200 sm:zoom-in-95 motion-reduce:animate-none',
      ...
      >
        <div className="flex items-start justify-between gap-4 border-b border-hairline px-6 py-4">
          ...
          <button
            type="button"
            onClick={onClose}
            className="shrink-0 rounded-control p-1.5 text-gray-400 transition-colors duration-fast ease-premium hover:bg-gray-100 hover:text-gray-950"
        ...
        {footer ? <div className="flex flex-wrap items-center justify-end gap-2 border-t border-hairline px-6 py-4">{footer}</div> : null}
      </div>
    </div>,
    document.body,
  );
  ...
  return createPortal(
    <div className="fixed inset-0 z-[90] flex justify-end" role="presentation">
      ...
      <div
        ...
        aria-label={title}
        className={cn(
          'relative flex h-full w-full flex-col bg-canvas shadow-overlay',
          'animate-in slide-in-from-right duration-200 motion-reduce:animate-none',
          slideWidth[width],
        )}
      >
        <div className="flex items-start justify-between gap-4 border-b border-hairline bg-white px-6 py-4">
          <div className="min-w-0">
        ...
        {footer ? <div className="flex flex-wrap items-center justify-end gap-2 border-t border-hairline bg-white px-6 py-4">{footer}</div> : null}
      </div>
    </div>,
    document.body,
  );
