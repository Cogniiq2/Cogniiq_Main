...
/**
 * RPCs that describe WHO the caller is rather than what data they have.
 *
 * "Empty" means the customer's organization holds no projects, documents or invoices — it
 * does not mean the customer has no access context. Blanking `current_user_portal_context`
 * would make every guarded route fail closed, so the harness would capture the guard's
 * loading/denial surface on every route and never reach the empty state it exists to
 * photograph. The guard is right to refuse; the fixture was wrong to ask.
 */
const SCENARIO_INVARIANT_RPC = new Set(['current_user_portal_context']);
...
export function resolve({ url, method = 'GET', headers = {}, scenario = 'populated', session, overrides = {} }) {
  ...
  if (path.startsWith('/rest/v1/rpc/')) {
    ...
    if (scenario === 'empty' && !SCENARIO_INVARIANT_RPC.has(name)) {
