...
async function main() {
  ...
  // ── phase 1d: explicit routing rules for every public URL ──
  // Generated from the same manifest that produced the pages, so the routing
  // table and the artifact cannot disagree. Forced ("!") so a canonical URL is
  // resolved before Netlify's pretty-URL normalisation is consulted at all —
  // that normalisation is what produced ERR_TOO_MANY_REDIRECTS on the previous
  // deployment. See public/_redirects for the full account.
  const MARKER = '# @generated-public-routes';
  ...
  if (!redirectsTemplate.includes(MARKER)) {
    throw new Error(`public/_redirects must contain the "${MARKER}" marker so route rules can be spliced in.`);
  }

  const ruleLines = [];
  for (const { route } of bodies) {
    if (route.path === '/') continue; // the root is served by dist/index.html directly
    const file = `${route.path}.html`;
    // Canonical URL -> its own document, with no redirect at all.
    ruleLines.push(`${route.path.padEnd(46)}${file.padEnd(50)}200!`);
    // The trailing-slash variant is not a second page: collapse it onto the
    // canonical with exactly ONE redirect. This can never loop, because the
    // target is itself resolved by the forced rewrite above.
    ruleLines.push(`${`${route.path}/`.padEnd(46)}${route.path.padEnd(50)}301!`);
  }
  const generated = [
    '',
    `# ${ruleLines.length / 2} public routes, generated by scripts/prerender.mjs — do not edit by hand.`,
    ...ruleLines,
    '',
  ].join('\n');
  ...
  rendered.push({
    ...
    outDir: DIST,
    file: redirectsPath,
    page: redirectsTemplate.replace(MARKER, () => `${MARKER}${generated}`),
  });
