...
async function main() {
  ...
  // ── phase 1d: Netlify-only canonical rewrites ──
  // On Netlify, cold requests to canonical URLs of the <path>.html artifact
  // returned ERR_TOO_MANY_REDIRECTS until each route got an explicit forced
  // rewrite. "!" is Netlify-specific syntax that Cloudflare's shared _redirects
  // format does not accept, so these rules are generated ONLY when the build is
  // running on Netlify and never written into the committed public/_redirects.
  //
  // Exactly one rule per non-root public route, and no trailing-slash rules:
  // Netlify states redirects cannot reliably add or remove a trailing slash, and
  // a forced "/route/ -> /route" rule can loop. "/route/" is left as a duplicate
  // variant consolidated by the canonical tag it already carries.
  const MARKER = '# @netlify-canonical-rewrites';
  const isNetlify = process.env.NETLIFY === 'true';
  const redirectsPath = join(DIST, '_redirects');
  if (!existsSync(redirectsPath)) throw new Error('dist/_redirects missing — the client build did not copy public/');
  const redirectsTemplate = readFileSync(redirectsPath, 'utf8');
  if (!redirectsTemplate.includes(MARKER)) {
    throw new Error(`public/_redirects must contain the "${MARKER}" marker so Netlify rules can be spliced in.`);
  }
  // Only RULE lines matter: comments are ignored by both hosts, and the marker
  // block above documents the "!" syntax without using it.
  const templateRules = redirectsTemplate
    .split('\n')
    .map((line) => line.trim())
    .filter((line) => line && !line.startsWith('#'));
  const forcedInTemplate = templateRules.filter((line) => line.includes('!'));
  if (forcedInTemplate.length) {
    throw new Error(
      `public/_redirects must not contain Netlify-only "!" syntax — Cloudflare rejects it. Found: ${forcedInTemplate.join(' | ')}`
    );
  }

  let redirectsOut = redirectsTemplate;
  let generatedRules = 0;
  if (isNetlify) {
    const lines = [];
    for (const { route } of bodies) {
      if (route.path === '/') continue; // the root is served by dist/index.html
      lines.push(`${route.path.padEnd(46)}${`${route.path}.html`.padEnd(50)}200!`);
    }
    generatedRules = lines.length;
    redirectsOut = redirectsTemplate.replace(
      MARKER,
      () =>
        `${MARKER}\n\n# ${lines.length} canonical rewrites generated for Netlify by scripts/prerender.mjs.\n${lines.join('\n')}\n`
    );
  }

  rendered.push({ path: '_redirects', outDir: DIST, file: redirectsPath, page: redirectsOut });
  ...
  writeFileSync(
    ...
    `${JSON.stringify(
      ...
      generatedNetlifyRules: generatedRules,
  ...
  console.log(
    `✓ Prerendered ${PUBLIC_ROUTES.length} public routes ` +
      ...
      `  _redirects: ${isNetlify ? `${generatedRules} Netlify canonical rewrites generated` : 'portable, no provider-specific rules'}\n` +
